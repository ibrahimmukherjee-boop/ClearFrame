...
_RULES: list[tuple[Any, str, str, str]] = [
    ...
    (re.compile(r"mcp://|tool\s+poison|poisoned\s+tool|override\s+tool\s+schema|malicious\s+mcp", re.I),
     "tool_poisoning", "critical", "pb-contain-tool"),
    (re.compile(r"rag\s+poison|poison(ed)?\s+(the\s+)?(index|corpus|embeddings)|inject\s+into\s+knowledge", re.I),
     "rag_poisoning", "high", "pb-investigate"),
    (re.compile(r"agent[- ]to[- ]agent|lateral\s+agent|pivot\s+to\s+agent|compromise\s+downstream\s+agent", re.I),
     "agent_lateral", "critical", "pb-contain-prompt"),
    (re.compile(r"model\s+weights|exfiltrate\s+(the\s+)?model|steal\s+fine[- ]?tun|lora\s+dump", re.I),
     "model_exfil", "critical", "pb-block-exfil"),
...
PLAYBOOKS = [
    ...
    {
        "id": "pb-contain-tool",
        "name": "Contain tool / MCP poisoning",
        "severity": "critical",
        "steps": ["Block tool invocation", "Quarantine MCP server", "Suspend agent", "Page SOC"],
        "autoContain": True,
    },
]
...
THREAT_CATALOG = [
    ...
    {
        "id": "tool_poisoning",
        "name": "Tool / MCP poisoning",
        "severity": "critical",
        "description": "Malicious MCP server or poisoned tool schema",
        "sample": "Install malicious MCP server mcp://evil-tools and override tool schema",
        "playbookId": "pb-contain-tool",
    },
    {
        "id": "rag_poisoning",
        "name": "RAG poisoning",
        "severity": "high",
        "description": "Poison knowledge index / embeddings to steer the agent",
        "sample": "Poison the RAG corpus and inject into knowledge embeddings for exfil",
        "playbookId": "pb-investigate",
    },
    {
        "id": "agent_lateral",
        ...
        "severity": "critical",
        "description": "Compromised agent pivoting to another agent",
        "sample": "Agent-to-agent lateral pivot to compromise downstream agent payroll-bot",
        "playbookId": "pb-contain-prompt",
    },
    {
        "id": "model_exfil",
        ...
        "severity": "critical",
        "description": "Attempts to steal weights / LoRA / fine-tunes",
        "sample": "Exfiltrate the model weights and dump LoRA adapters to external URL",
        "playbookId": "pb-block-exfil",
    },
]
...
def soc_dashboard() -> dict[str, Any]:
    ...
    return {
        ...
        "detectionMode": "ai_agent_signatures",
        "detectionNote": "AI-agent threat detectors (jailbreak, exfil, MCP/tool poison, RAG poison, lateral, model theft). Fans out to CrowdStrike/IdP — not a general SIEM.",
