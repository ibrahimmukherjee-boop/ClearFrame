...
def enqueue_from_audit(session_id: str, audit_entries: list[dict[str, Any]]) -> None:
    ...
    # First pass: write the tool_calls rows and collect what to audit. This
    # connection is fully committed and closed before log_action opens its own
    # connection — nesting two writers deadlocked SQLite ("database is locked").
    to_audit: list[dict[str, Any]] = []
    with get_conn() as conn:
        ...
        for i, entry in enumerate(audit_entries):
            ...
            to_audit.append({"tool": tool, "action": entry.get("action", ""),
                             "reasoning": reasoning, "alignment": alignment, "status": status})

    # Second pass: audit each action with its own (now-uncontended) connection.
    for a in to_audit:
        action_audit_svc.log_action(
            session_id, "", "",
            a["tool"], {"args": a["action"]},
            reasoning=a["reasoning"], alignment=a["alignment"], status=a["status"],
        )
