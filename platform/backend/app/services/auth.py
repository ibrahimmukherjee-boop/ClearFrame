...
def init_auth_db() -> None:
    with get_conn() as conn:
        ...
        if not count:
            ...
            if is_production() and not admin_pw:
                raise RuntimeError("CLEARFRAME_ADMIN_PASSWORD required in production (no default users)")
            if admin_pw:
                # Honour a provided admin password in any mode so a public
                # deployment is never seeded with admin/admin.
                op_pw = os.environ.get("CLEARFRAME_OPERATOR_PASSWORD", admin_pw)
                aud_pw = os.environ.get("CLEARFRAME_AUDITOR_PASSWORD", admin_pw)
                defaults = [
                ...
                # Local dev only — no admin password configured.
                defaults = [
